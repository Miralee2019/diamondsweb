      ...
      <div class="span12">
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5>View Product</h5>
            <span style="float:right;" class="icon"><a href="<?php echo base_url('index.php/admin/Admin/addsale_product'); ?>"><i class="icon-plus" title="Add Sale product"></i></a></span>
          </div>
          <div class="widget-content nopadding">
              ...
              <tbody>
                  <?php if($row != null){ 
                    ...
                    function searchForId($search_value, $array, $id_path) {
                      ...
                      foreach($row['product'] as $data){ //echo "<pre>"; print_r($row['sale_product']);
                    foreach($data as $rows){
                      $salePercentage;
                      $saleid;
                      foreach($row['sale_product'] as $sale){
                ...
                <tr class="odd gradeX">
                    ...
                    <td><?php $img = explode(',',$rows['image']); foreach ($img as $m) { ?>
                     <img src="<?php echo base_url('assets/images/'.$m);?>" style="width:100px;">
                      <?php } ?></td>
                    <td><?php echo $rows['weight']; ?></td>
                    <td><?php echo $salePercentage; ?></td>
                    <!-- <td align="center"><a href="<?php echo site_url('admin/Admin/view_sale/'.$sale['sale_id'].'/1'); ?>"><?php if($rows['sale']=='0'){?>
                        <img src="<?php echo base_url('assets/images/disapprove.png')?>" width="30px"></a><?php }else { ?>
                        <a  href="<?php echo site_url('admin/Admin/view_sale/'.$rows['p_id'].'/0'); ?>">
                        <img src="<?php echo base_url('assets/images/approve.png')?>" width="30px"/></a><?php } ?>
                    </td> -->
                    <td align="center"><a href="<?php echo site_url('admin/Admin/delete_saleproduct/').@$saleid?>" />
                        <img src=<?php echo base_url ('assets/images/delete.png');?> width="30" height="30" /></a>
                    </td>   
                    <td align="center"><a href="<?php echo site_url('admin/Admin/update_product/'.@$saleid); ?>" />
            ...
            <center>
              <?php //print_r(count($row['sale_product'])); ?>
					    <?php //if(count($row['sale_product']) > 5) {echo @$pagination;}?>
            </center>
          </div>
        </div>
      </div>
